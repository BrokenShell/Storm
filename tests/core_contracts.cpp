...
void test_version_and_types() {
    ...
    static_assert(std::is_same_v<decltype(Storm::canonical()), Storm::Float>);
    static_assert(std::is_same_v<decltype(Storm::uniform_integer(0, 1)), Integer>);
    static_assert(std::is_same_v<decltype(Storm::uniform_unsigned(Unsigned{0}, Unsigned{1})),
                                 Unsigned>);
    static_assert(std::is_same_v<decltype(Storm::uniform_index(std::size_t{1})), std::size_t>);
    static_assert(std::is_same_v<decltype(Storm::random_range(0, 1, 1)), Integer>);
    ...
    static_assert(std::is_same_v<decltype(Storm::roll_dice(std::size_t{1}, std::size_t{1})),
                                 Unsigned>);
    static_assert(std::is_same_v<decltype(Storm::ability_dice(std::size_t{3})), Unsigned>);
    static_assert(!noexcept(Storm::uniform_integer(0, 1)));
    static_assert(!noexcept(Storm::uniform_unsigned(Unsigned{0}, Unsigned{1})));
...
void test_owned_and_injected_engines() {
    Storm::Generator first{Unsigned{0}};
    Storm::Generator second{Unsigned{0}};
    static_assert(std::is_same_v<decltype(first.engine()), Storm::engine_type&>);

    for (int index = 0; index < 32; ++index) {
        STORM_CHECK(Storm::uniform_unsigned(first.engine(), Unsigned{0}, unsigned_max) ==
                    Storm::uniform_unsigned(second.engine(), Unsigned{0}, unsigned_max));
    }

    first.seed(Unsigned{73});
    second.seed(Unsigned{73});
    ...
    STORM_CHECK(Storm::uniform_unsigned(first.engine(), Unsigned{0}, unsigned_max) ==
                Storm::uniform_unsigned(copied.engine(), Unsigned{0}, unsigned_max));
...
void test_tls_seed_zero() {
    std::array<Unsigned, 16> first{};
    std::array<Unsigned, 16> second{};
    Storm::seed(Unsigned{0});
    for (auto& value : first) {
        value = Storm::uniform_unsigned(Unsigned{0}, unsigned_max);
    }
    Storm::seed(Unsigned{0});
...
void test_v5_bounded_golden_vectors() {
    constexpr std::array<Unsigned, 8> expected_unsigned{
        4ULL, 7ULL, 3ULL, 8ULL, 6ULL, 8ULL, 3ULL, 4ULL,
    };
    constexpr std::array<Integer, 8> expected_integer{
        3LL, 0LL, -3LL, 1LL, -5LL, -1LL, -4LL, -1LL,
    };
    constexpr std::array<Unsigned, 8> expected_full_unsigned{
    ...
    constexpr std::array<Integer, 8> expected_full_integer{
    ...
    constexpr std::array<Integer, 8> expected_range{
    ...
    Storm::Generator generator{Unsigned{0}};
    for (const Unsigned expected : expected_unsigned) {
        STORM_CHECK(Storm::uniform_unsigned(generator.engine(), Unsigned{0}, Unsigned{9}) ==
                    expected);
    }
    generator.seed(Unsigned{0});
    for (const Integer expected : expected_integer) {
        STORM_CHECK(Storm::uniform_integer(generator.engine(), -5, 5) == expected);
    }
    generator.seed(Unsigned{0});
    for (const Unsigned expected : expected_full_unsigned) {
        STORM_CHECK(Storm::uniform_unsigned(generator.engine(), Unsigned{0}, unsigned_max) ==
                    expected);
    }
    generator.seed(Unsigned{0});
    for (const Integer expected : expected_full_integer) {
    ...
    generator.seed(Unsigned{0});
    for (const Integer expected : expected_range) {
...
void test_uniform_integers() {
    ...
    STORM_CHECK(Storm::uniform_unsigned(generator.engine(), Unsigned{0}, Unsigned{0}) == Unsigned{0});
    STORM_CHECK(Storm::uniform_unsigned(generator.engine(), unsigned_max, unsigned_max) == unsigned_max);

    for (int index = 0; index < 4096; ++index) {
        const Integer signed_value = Storm::uniform_integer(generator.engine(), integer_min, integer_max);
        const Unsigned unsigned_value =
            Storm::uniform_unsigned(generator.engine(), Unsigned{0}, unsigned_max);
    ...
    STORM_EXPECT_THROWS(std::invalid_argument,
                        Storm::uniform_unsigned(generator.engine(), Unsigned{1}, Unsigned{0}));
    STORM_CHECK(generator.engine() == state_before_invalid);
}

void test_uniform_index() {
    Storm::Generator generator{Unsigned{202}};
    ...
    Storm::seed(Unsigned{202});
    STORM_CHECK(Storm::uniform_index(std::size_t{17}) < 17U);
}

void test_directed_ranges() {
    Storm::Generator generator{Unsigned{303}};
    for (int index = 0; index < 2048; ++index) {
        const Integer ascending = Storm::random_range(generator.engine(), 1, 10, 2);
        STORM_CHECK(ascending >= 1 && ascending < 10);
        STORM_CHECK((ascending - 1) % 2 == 0);

        const Integer descending = Storm::random_range(generator.engine(), 10, 1, -2);
        STORM_CHECK(descending <= 10 && descending > 1);
        STORM_CHECK((10 - descending) % 2 == 0);
    }

    for (int index = 0; index < 256; ++index) {
        const Integer wide = Storm::random_range(generator.engine(), integer_min, integer_max, integer_max);
        STORM_CHECK(wide == integer_min || wide == -1 || wide == integer_max - 1);

        const Integer huge_step = Storm::random_range(generator.engine(), integer_max, integer_min, integer_min);
    ...
    Storm::seed(Unsigned{303});
    const Integer tls_value = Storm::random_range(1, 10, 2);
    STORM_CHECK(tls_value >= 1 && tls_value < 10 && (tls_value - 1) % 2 == 0);
}

void test_dice() {
    Storm::Generator generator{Unsigned{404}};
    ...
    const Unsigned maximum_single_roll =
        Storm::roll_dice(generator.engine(), std::size_t{1},
                         std::numeric_limits<std::size_t>::max());
    STORM_CHECK(maximum_single_roll >= Unsigned{1});
    for (int index = 0; index < 1024; ++index) {
        const std::size_t die = Storm::roll_die(generator.engine(), std::size_t{20});
        const Unsigned dice = Storm::roll_dice(generator.engine(), std::size_t{3}, std::size_t{6});
        const Unsigned ability = Storm::ability_dice(generator.engine(), std::size_t{10});
